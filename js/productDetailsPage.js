...
async function displayProductDetails(url) {
    try {
        ...
        <p><strong>Discount Percentage:</strong> ${response?.discountPercentage?.toFixed(2)}%</p>
        <p><strong>Rating:</strong> ${"⭐".repeat(Math.floor(response?.rating))} ${response?.rating}</p>
        <p><strong>Stock:</strong> ${response?.stock}</p>
        <p><strong>Brand:</strong> ${response?.brand}</p>
        <p><strong>Category:</strong> ${response?.category}</p>
        <a data-productId="${response?.id}" onclick="addToCart(${response?.id});" class="add-to-cart">add-to-cart</a>
        </div>
